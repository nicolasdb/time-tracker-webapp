...
**Tasks**:
- [ ] Set up Streamlit container with Docker
- [ ] Configure Supabase connection
- [ ] Create basic UI layout
- [ ] Implement test data display
...
CREATE TABLE devices (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  uid_device TEXT UNIQUE NOT NULL,
  device_name TEXT,
...
FROM 
  time_events e1
JOIN 
  time_events e2 ON e1.uid_tag = e2.uid_tag AND e1.id != e2.id
JOIN
  tags t ON e1.uid_tag = t.uid_tag
WHERE 
  e1.event_type = 'insert_tag' AND
  e2.event_type = 'remove_tag' AND
  e2.timestamp > e1.timestamp AND
  NOT EXISTS (
    SELECT 1 FROM time_events e3
    WHERE e3.uid_tag = e1.uid_tag 
